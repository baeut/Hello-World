...
volatile u16 buf01=0;

volatile u8 recv_buf[16] = {0};
volatile u8 state = 0, write_offset = 0, frame_available = 0;
...
//	buf[8] = {};
	...
	if(USART_GetITStatus(USART2,USART_IT_RXNE)) // 中断标志
 {
	 ...
	 if (state == 0) {
		 ...
		 if (res == 0) {
			 recv_buf[write_offset] = 0;
			 write_offset = 0;
			 state = 0;
			 frame_available = 1;
		 } else {
			 recv_buf[write_offset++] = res;
		 }
	 } else {
		 state = 0;
		 write_offset = 0;
